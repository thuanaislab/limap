...
###############################################################################
# The following utils functions are taken/modified from hloc.pipelines.indoor6
###############################################################################
...
def read_scene_indoor6(cfg, root_path, model_path, image_path, n_neighbors=20):
    metainfos_filename = 'infos_indoor6.npy'
...
def run_hloc_indoor6(cfg, dataset, scene, results_file, test_list, num_covis=30, use_dense_depth=False, logger=None):
    ...
    depth_dir = dataset / f'depth/indoor6_{scene}/train/depth'
    retrieval_path = dataset / f'{scene}_netvlad_top10.txt'
    feature_conf = {
    ...
    if not (cfg['skip_exists'] or cfg['localization']['hloc']['skip_exists']) or not os.path.exists(results_file):
        ...
        localize_sfm.main(
            ref_sfm, query_list, retrieval_path, features, loc_matches, results_file, covisibility_clustering=False, prepend_camera_name=False)
