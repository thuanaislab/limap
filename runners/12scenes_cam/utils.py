...
def read_scene_12scenes(cfg, root_path, n_neighbors=20):
...
def get_scene_info(vsfm_path, imagecols):
    ...
    for id in imagecols.get_img_ids():
        ...
        image_name = imagecols.image_name(id).split('/')[-2] + '/' + imagecols.image_name(id).split('/')[-1]
...
def run_hloc_12scenes(cfg, image_dir, imagecols, neighbors, train_ids, query_ids, id_to_origin_name,
